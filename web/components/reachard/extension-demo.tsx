...
export function ExtensionDemo() {
  ...
  const position = reducedMotion ? 10000 : time;
  const scene = position < 3600 ? 'role' : position < 6200 ? 'search' : position < 12700 ? 'people' : 'draft';
  const reasonVisible = position > 8100;
  const draftLength = Math.max(0, Math.min(DRAFT.length, Math.floor((position - 13700) / 13)));
    ...
    <figure ref={container} className="rh-demo" data-scene={scene} data-paused={paused || reducedMotion} aria-label="Illustrative Reachard extension demo: open a job, find relevant people and draft a personal email.">
      <div className="rh-collage" aria-hidden="true">
        ...
        <div className="rh-extension">
          ...
          <div className="rh-extension-body">
            <div className="rh-demo-scene rh-role-scene" data-active={scene === 'role' || scene === 'search'}>
              ...
              <div className="rh-demo-search" data-loading={scene === 'search'}>{scene === 'search' ? <><span className="rh-loading-spinner" />Finding people…</> : <>Find people at Stripe <ArrowRight size={17} /></>}</div>
            ...
            <div className="rh-demo-scene rh-people-scene" data-active={scene === 'people'}>
              ...
              <div className="rh-contact-main"><Person index={0} /><div className="rh-match" data-visible={reasonVisible}><span><Sparkles size={13} />Why reach out</span><p>Leads product design on the team you’re exploring. A direct perspective on the role and what you could work on.</p></div><div className="rh-demo-write"><Mail size={14} />Get email & draft <ArrowRight size={14} /></div></div>
...
function Person({ index }: { index: number }) {
  ...
  return <div className="rh-person"><Avatar className="rh-avatar"><Avatar.Image src={`/images/workspace/${person.image}.png`} alt="" /><Avatar.Fallback>{person.name.charAt(0)}</Avatar.Fallback></Avatar><div><strong>{person.name}</strong><span>{person.title}</span></div><ArrowRight size={13} /></div>;
}
