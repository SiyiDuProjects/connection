...
export function ExtensionDemo() {
  ...
  useEffect(() => {
    ...
    // Expanding the match changes the draft button's position without changing
    // the button's own size, so also measure its containing contact row.
    [panel, searchTarget.current, personTarget.current, draftTarget.current, draftTarget.current?.parentElement].forEach(element => { if (element) observer.observe(element); });
    measure();
  ...
  const clicking = position >= clickStart && position <= clickStart + 240;
        ...
        <div className="rh-extension">
          ...
          <div ref={body} className="rh-extension-body">
            ...
            {pointer && <div className="rh-demo-pointer" data-target={pointerTarget} data-clicking={clicking} style={{ transform: `translate(${pointer.x}px, ${pointer.y}px)`, opacity: pointerVisible ? 1 : 0 }}>
              {/* macOS default cursor geometry: https://mac-cursors.netlify.app/svg/default.svg */}
