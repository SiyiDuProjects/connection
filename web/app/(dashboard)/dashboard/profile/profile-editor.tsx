...
export default function ProfileEditor({ preview = false }: { preview?: boolean }) {
  const { data, mutate } = useSWR<AccountData>(preview ? null : '/api/account', fetcher);
  const { data: inviteData, mutate: mutateInvite } = useSWR<InviteData>(preview ? null : '/api/invite-friend', fetcher);
  ...
  return (
    <fieldset disabled={preview} className="dashboard-page-content dashboard-overview">
      ...
      <DashboardCard id="resume" title={t('dashboard.resume')}>
        ...
        >
          <p>
            {t('dashboard.lastUploaded', { value: settings.resumeUploadedAt ? formatDateTime(settings.resumeUploadedAt) : t('dashboard.notAvailable') })}
          </p>
...
function InviteFriendBanner({
  ...
  return (
    <section className="mx-auto mt-4 max-w-5xl rounded-[16px] border border-border bg-card px-6 py-6 shadow-apple-card">
...
function SectionIndex() {
  ...
  return (
    <nav className="mx-auto mt-4 flex max-w-5xl flex-wrap items-start justify-start gap-x-3 gap-y-2" aria-label={t('dashboard.sections')}>
      {items.map((item) => {
        ...
        return (
          <a
            key={item.href}
            href={item.href}
            className="group flex items-center gap-2 rounded-full bg-default px-4 py-2 text-sm font-medium"
          >
            <Icon className="h-4 w-4 text-muted transition-colors group-hover:text-[#1d1d1f]" strokeWidth={1.8} aria-hidden="true" />
...
function DashboardCard({
  ...
  return (
    <section id={id} className="mx-auto mt-4 max-w-5xl scroll-mt-8 rounded-[16px] border border-border bg-card px-6 py-6 shadow-apple-card">
