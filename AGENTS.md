...
- Current monthly pricing is **Base $9: 50 successfully verified work-email unlocks per billing period, without rollover**, and **Plus $19: personal unlimited, with no numerical monthly cap**. Search and drafts cost no app credits; failed reveals and reopening an already unlocked email cost no app credits. Grandfathered $8/$12 Stripe prices retain their purchased 20/60-credit rules.
- Website and contacts service read `get_account_entitlement(user_id)` from migration `0019_plan_entitlements`. Paid grants carry immutable purchased price and billing-period evidence; never derive unlimited access from a product display name or an unpaid subscription update. Upgrades require a paid invoice; portal downgrades are scheduled at period end. Keep persistent email unlock records in account deletion cleanup.
